...
const topicOptionsContainer = document.getElementById(
...
const scoreFeedbackMessageEl = document.getElementById(
  "score-feedback-message"
)!;
const summaryTotalQuestionsEl = document.getElementById(
...
const scoreMainContent = document.querySelector(
  "#score-page .score-main-content"
) as HTMLElement;
...
function showQuestion() {
  ...
  questionTextEl.textContent = `${currentQuestionIndex + 1}. ${
    question.question
  }`;

  answerOptionsContainer.innerHTML = "";
  (skipQuestionBtn as HTMLButtonElement).hidden = false;
  question.options.forEach((optionText) => {
    ...
    input.addEventListener("change", () =>
      handleAnswerSelection(input, question.correctAnswer)
    );
...
function handleAnswerSelection(selectedInput: HTMLInputElement, correctAnswer: string) {
    clearInterval(timer);
    
    answerOptionsContainer
        .querySelectorAll('input[type="radio"]')
        .forEach((radio) => ((radio as HTMLInputElement).disabled = true));

    // Add this line to disable the skip button
    (skipQuestionBtn as HTMLButtonElement).hidden = true;
...
function resetQuiz() {
  ...
  userNameInput.value = "";
  const checkedTopic = document.querySelector(
    'input[name="topic"]:checked'
  ) as HTMLInputElement;
  if (checkedTopic) {
